...
import type {
  ...
  DraftLessonDto,
  DraftTaskDto,
} from "../../api/courseDraftApi.ts";
import type {
  TeacherStatsDto,
  TeacherCourseStatsDto,
} from "../../api/types/teacher.ts";
...
const TeacherStatsTab = () => {
  ...
  const cards = [
    ...
    {
      label: "Заработок",
      value: `${stats.totalEarnings.toLocaleString("ru-RU", { maximumFractionDigits: 0 })} ₽`,
      sub: `+${stats.earningsThisMonth.toLocaleString("ru-RU", { maximumFractionDigits: 0 })} ₽ в этом месяце`,
      color: "rgba(0,100,0,0.85)",
    },
    {
      label: "Сертификатов",
              ...
              {courseStats.map((course, i) => (
                ...
                >
                  ...
                  <td style={{ padding: "12px 16px", textAlign: "right" }}>
                    <Text
                      strong
                      style={{ fontSize: 15, color: "rgba(0,100,0,0.85)" }}
                    >
                  ...
                  <td style={{ padding: "12px 16px", textAlign: "right" }}>
                    <Text
                      ...
                      style={{
                        fontSize: 15,
                        color: course.rating > 0 ? "#d46b08" : undefined,
                      }}
                    >
                      {course.rating > 0
...
const TaskForm = ({
  ...
  const addHint = () => {
    if (hints.length < 3) setHints((p) => [...p, ""]);
  };
  const removeHint = (i: number) =>
    setHints((p) => p.filter((_, idx) => idx !== i));
  const setHint = (i: number, val: string) =>
    setHints((p) => p.map((h, idx) => (idx === i ? val : h)));
  const updateOption = (i: number, val: string) =>
    setOptions((p) => p.map((o, idx) => (idx === i ? val : o)));
  const addOption = () => {
    if (!options[options.length - 1].trim()) {
      message.warning("Заполните предыдущий вариант");
      return;
    }
    if (options.length >= MAX_OPTIONS) {
      message.warning(`Максимум ${MAX_OPTIONS}`);
      return;
    }
    setOptions((p) => [...p, ""]);
  };
  ...
  const handleSubmit = async (values: TaskFormValues) => {
    setSubmitted(true);
    if (taskType !== "TextInput") {
      const emptyIndex = options.findIndex((o) => !o.trim());
      if (emptyIndex !== -1) {
        message.error(`Вариант ${emptyIndex + 1} не заполнен`);
        return;
      }
    }
    setSaving(true);
    ...
    if (draftMode) {
      const draftTask: DraftTaskDto = {
        ...
        correctIndex:
          taskType === "SingleChoice"
            ? (values.correctIndex ?? undefined)
            : undefined,
        correctIndexes:
          taskType === "MultiChoice"
            ? (values.correctIndexes ?? undefined)
            : undefined,
        correctAnswer:
          taskType === "TextInput"
      ...
      onCreated(draftTask as unknown as TaskTeacherDto);
      form.resetFields();
      setOptions(["", ""]);
      setHints([""]);
      setSubmitted(false);
    ...
    if (taskType === "SingleChoice") {
      if (values.correctIndex == null) {
        message.error("Выберите правильный ответ");
        setSaving(false);
        return;
      }
      ...
      if (!values.correctIndexes?.length) {
      ...
      result = await taskTeacherApi.createMultiChoice(lessonId, {
        question: values.question,
        options,
        correctIndexes: values.correctIndexes,
        explanation: values.explanation,
        hints: cleanHints,
        points: values.points ?? 1,
        isRequired: values.isRequired ?? true,
      ...
      >
        <Form.Item label="Тип задания">
          ...
          >
            ...
            <Select.Option value="MultiChoice">
              Множественный выбор
            </Select.Option>
        ...
        <Form.Item label="Объяснение" name="explanation">
          <Input.TextArea rows={2} />
        </Form.Item>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <Button onClick={draftMode ? () => setShowForm(false) : onCancel}>
            Отмена
          </Button>
        </div>
      </Form>
...
const LessonEditor = ({
  ...
  const handleSave = async (values: LessonFormValues) => {
    ...
    if (lesson) {
      ...
      if (ok) {
        message.success("Урок обновлён");
        onSaved({ ...lesson, ...payload }, false);
      } else {
  ...
  return (
    ...
    >
      ...
      <Form form={form} layout="vertical" onFinish={handleSave}>
        ...
        <Form.Item label="Содержимое (Markdown)">
          <MarkdownEditor
            value={content}
            onChange={setContent}
            minHeight={400}
    ...
    >
      ...
      >
        ...
        <Space onClick={(e) => e.stopPropagation()}>
          <AntTooltip title="Редактировать">
            <Button
              size="small"
              icon={<EditOutlined />}
              onClick={() => {
      ...
      {tasks.map((task, idx) => (
        ...
        >
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            ...
            <div>
              <Tag color="blue" style={{ fontSize: 11, marginBottom: 6 }}>
                {taskTypeLabel[task.type]}
              </Tag>
              ...
              {task.options.map((opt, i) => (
                ...
                >
                  ...
                  <Text
                    style={{
                      fontSize: 13,
                      color: task.correctIndex === i ? "#389e0d" : undefined,
                    }}
                  >
                    {opt}
                  </Text>
          ...
          {task.type === "MultiChoice" && task.options && (
            ...
            >
              {task.options.map((opt, i) => {
                const isC = task.correctIndexes?.includes(i);
                return (
                  <div
                    key={i}
                    style={{
                      padding: "6px 10px",
                      borderRadius: 6,
                      background: isC ? "#f6ffed" : "#fafafa",
                      border: `1px solid ${isC ? "#b7eb8f" : "#f0f0f0"}`,
                      display: "flex",
                      alignItems: "center",
                      gap: 8,
                    }}
...
// ─── DraftLessonEditor ───────────────────────────────────────
const DraftLessonEditor = ({
  lesson,
  onSave,
  onCancel,
}: {
  lesson: DraftLessonDto | null;
  onSave: (data: {
    title: string;
    description: string;
    content: string;
...
const CourseEditor = ({
  ...
  const [inlineLesson, setInlineLesson] = useState<{
    sectionId: string;
  } | null>(null);
  ...
  const [existingDraft, setExistingDraft] = useState<CourseDraftDto | null>(
    null,
  );
  ...
  const [editingDraftSection, setEditingDraftSection] =
    useState<DraftSectionDto | null>(null);
  const [draftSectionForm] = Form.useForm<SectionFormValues>();
  const [editingDraftLesson, setEditingDraftLesson] = useState<{
    sectionId: string;
    lesson: DraftLessonDto | null;
  } | null>(null);
  ...
  useEffect(() => {
    if (draftSectionModal) {
      ...
      else draftSectionForm.resetFields();
    }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      await Promise.all(
        data.map(async (s) => {
          lm[s.id] = await sectionApi.getLessonsBySection(s.id);
        }),
      );
      setLessonsBySection(lm);
      setLoading(false);
    };
    void load();
  }, [course.id]);
  ...
  const saveDraftToServer = async (
    ...
    setSavingDraft(true);
    const result = await courseDraftApi.updateDraftFull(
      course.id,
      draft.id,
      draftToCreateDto({ ...draft, sections }),
    );
  ...
  const updateDraftState = (saved: CourseDraftDto) => {
    setExistingDraft(saved);
    setDraftSections(
      [...(saved.sections ?? [])].sort((a, b) => a.orderIndex - b.orderIndex),
    );
  };

  const handleDraftSaveSection = async (values: SectionFormValues) => {
    if (!existingDraft) return;
    const newSections = editingDraftSection
      ? draftSections.map((s) =>
    ...
    if (saved) {
      ...
      message.success(
        editingDraftSection ? "Раздел обновлён" : "Раздел создан",
      );
    }
  ...
  const handleDraftSaveLesson = async (
    sectionId: string,
    ...
    if (saved) {
      updateDraftState(saved);
      message.success(
        editingDraftLesson?.lesson ? "Урок обновлён" : "Урок создан",
      );
    }
    setEditingDraftLesson(null);
  };

  const handleDraftDeleteLesson = async (
    sectionId: string,
    lessonId: string,
  ) => {
    if (!existingDraft) return;
  ...
  const handleDraftTaskCreated = async (
    sectionId: string,
    lessonId: string,
    task: DraftTaskDto,
  ) => {
            ...
            lessons: s.lessons.map((l) =>
              l.id !== lessonId
                ...
                : {
                    ...
                    tasks: [
                      ...l.tasks,
                      { ...task, orderIndex: l.tasks.length },
                    ],
                  },
            ),
          },
  ...
  const handleDraftTaskDeleted = async (
    sectionId: string,
    lessonId: string,
    taskId: string,
  ) => {
  ...
  const handleSaveSection = async (values: SectionFormValues) => {
    if (editingSection) {
      ...
      if (ok) {
        setSections((p) =>
          p.map((s) => (s.id === editingSection.id ? { ...s, ...values } : s)),
        );
  ...
  const handleDeleteLesson = async (lessonId: string) => {
    ...
    if (ok) {
      setLessonsBySection((p) => {
        const n = { ...p };
        for (const s in n) n[s] = n[s].filter((l) => l.id !== lessonId);
        return n;
      });
      message.success("Удалён");
    }
  ...
  const handleSubmitModeration = async () => {
    ...
    if (ok) {
      ...
      onUpdated({ ...course, status: "UnderReview" });
    } else {
      message.error("Ошибка");
    }
    setSubmitting(false);
  ...
  const handleSaveCourse = async (values: CourseFormValues, tags: string[]) => {
    ...
    const ok = await courseApi.updateCourse(course.id, {
      ...values,
      tags: tags.map((name) => ({ name })),
    });
    if (ok) {
      message.success("Обновлён");
      onUpdated({ ...course, ...values, tags: tags.map((name) => ({ name })) });
  ...
  const handleSaveCourseDraft = async (
    ...
    if (!existingDraft) return;
    setSavingCourse(true);
    const saved = await courseDraftApi.updateDraftFull(
      course.id,
      existingDraft.id,
      draftToCreateDto({
        ...existingDraft,
    ...
    if (saved) {
      updateDraftState(saved);
      message.success("Сохранено в черновик");
      setEditCourseOpen(false);
    } else {
      message.error("Ошибка");
    }
  ...
  const handleConfirmEdit = async () => {
    ...
    if (draft) {
      ...
      message.error("Не удалось создать черновик");
    }
  };
  const openEditForm = () => {
    ...
    courseForm.setFieldsValue({
      ...
      categoryId:
        (source as CourseDto).categoryId ??
        (source as CourseDraftDto).categoryId ??
        undefined,
    ...
    <>
      ...
      >
        <Title level={4} style={{ margin: 0 }}>
          Разделы курса{" "}
          {savingDraft && <Spin size="small" style={{ marginLeft: 8 }} />}
        </Title>
        <Button
          icon={<PlusOutlined />}
          onClick={() => {
            setEditingDraftSection(null);
      ...
      {draftSections.length === 0 ? (
        ...
        <Collapse
          key={draftSections.map((s) => s.id).join(",")}
          defaultActiveKey={draftSections.map((s) => s.id)}
        >
          {[...draftSections]
            ...
            .map((section) => (
              ...
              >
                    ...
                    <div key={lesson.id}>
                      {editingDraftLesson?.lesson?.id === lesson.id ? (
                        <DraftLessonEditor
                          lesson={lesson}
                          onSave={(data) =>
                            handleDraftSaveLesson(section.id, data)
                          }
                          ...
                          >
                            ...
                            <Space>
                              ...
                              <Popconfirm
                                title="Удалить урок?"
                                onConfirm={() =>
                                  handleDraftDeleteLesson(section.id, lesson.id)
                                }
                                okText="Удалить"
                                cancelText="Отмена"
                                okButtonProps={{ danger: true }}
                ...
                {editingDraftLesson?.sectionId === section.id &&
                !editingDraftLesson.lesson ? (
                  <DraftLessonEditor
                    lesson={null}
      ...
      <Modal
        open={draftSectionModal}
        title={editingDraftSection ? "Редактировать раздел" : "Новый раздел"}
        onCancel={() => setDraftSectionModal(false)}
        footer={null}
        centered
      >
        <Form
          form={draftSectionForm}
          layout="vertical"
          onFinish={handleDraftSaveSection}
        >
          <Form.Item
            label="Название"
            name="title"
  ...
  return (
    <div>
      ...
      >
        ...
        <Tag color={statusColor[course.status]}>
      ...
      {course.status === "RejectedByModerator" && (
        <Alert
          type="error"
          message="Курс отклонён модератором"
      ...
      >
        {!isReadOnly && !isDraftReview && (
          <AntTooltip
            title={
              isUnderReview ? "Нельзя редактировать во время модерации" : ""
            }
          >
            <Button
              icon={<EditOutlined />}
        ...
        {isEditing && (
          <>
            ...
            <Popconfirm
              ...
              okButtonProps={{ danger: true }}
            >
              <Button danger loading={cancellingDraft}>
                Отменить изменения
              </Button>
            </Popconfirm>
          </>
      ...
      {loading ? (
        ...
        >
          ...
          >
            {isUnderReview ? "Курс на проверке" : "Изменения на проверке"}
          </Text>
          <Text type="secondary" style={{ fontSize: 14 }}>
            {isUnderReview
              ? "До завершения проверки редактирование недоступно."
              : "Студенты видят текущую версию. После одобрения изменения применятся автоматически."}
          </Text>
        </div>
      ) : isReadOnly ? (
        <>
          <Alert
            ...
            description="Чтобы внести изменения — нажмите «Начать редактирование»."
            showIcon
            style={{ marginBottom: 16 }}
            action={
              <Button
          ...
          {sections.length === 0 ? (
            <Empty description="Разделов нет" />
          ) : (
            <Collapse>
              {sections.map((s) => (
                <Panel
                ...
                >
                  {(lessonsBySection[s.id] ?? []).length === 0 ? (
                    <Text type="secondary">Уроков нет</Text>
                  ) : (
                    <Collapse>
                      {(lessonsBySection[s.id] ?? []).map((l) => (
                        ...
                        >
                          <ReadonlyLessonContent lessonId={l.id} />
                        </Panel>
                      ))}
                    </Collapse>
                  )}
      ...
      <Modal
        ...
        title={
          isEditing
            ? "Редактировать метаданные (черновик)"
            : "Редактировать курс"
        }
      ...
      >
        {isEditing && (
          <Alert
            type="info"
            message="Изменения сохраняются в черновик."
            showIcon
            style={{ marginBottom: 16 }}
          />
        )}
      ...
      >
        <div style={{ padding: "8px 0 16px" }}>
          <Text style={{ display: "block", marginBottom: 16 }}>
            Изменения пройдут проверку модератора перед публикацией.
          </Text>
          <Text type="secondary" style={{ display: "block", marginBottom: 24 }}>
            Студенты видят текущую версию пока идёт проверка.
          </Text>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <Button onClick={() => setConfirmEditModalOpen(false)}>
              Только просмотр
            </Button>
            <Button
              type="primary"
...
const TeacherPage = () => {
  ...
  const counts: Record<string, number> = {
    ...
    RejectedByModerator: allCourses.filter(
      (c) => c.status === "RejectedByModerator",
    ).length,
  ...
  return (
    <>
      <Header />
      <Layout
        style={{ minHeight: "calc(100vh - 64px)", background: "#fafafa" }}
      >
            ...
            <>
              ...
              >
                <div>
                  ...
                  <Text type="secondary">
                    Всего курсов: <strong>{allCourses.length}</strong>
                  </Text>
                </div>
                {activeTab === "courses" && (
                  <Button
                    ...
                    >
                      {(
                        [
                          ...
                          {
                            value: "RejectedByModerator",
                            label: "Отклонённые",
                          },
                          { value: "Published", label: "Опубликованные" },
                        ] as { value: CourseStatus | ""; label: string }[]
                      ).map((btn) => (
                        <Button
                          key={btn.value}
                          type={
                            statusFilter === btn.value ? "primary" : "default"
                          }
                          onClick={() => {
                              ...
                              style={{
                                ...
                                background:
                                  statusFilter === btn.value
                                    ...
                                    : "#f0f0f0",
                                color:
                                  statusFilter === btn.value ? "#fff" : "#666",
                                borderRadius: 10,
                                padding: "0 6px",
                                fontSize: 11,
                                fontWeight: 600,
